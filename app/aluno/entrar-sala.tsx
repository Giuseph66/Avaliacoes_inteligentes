...
export default function EntrarSala() {
  ...
  async function handleEntrar(cod?: string) {
    ...
    const alunoSnap = await safeFirestoreOperation(() => getDoc(alunoDoc), 'verificar status do aluno');
    setCodigo('');
    setScannedData(null);

    if (alunoSnap && alunoSnap.exists()) {
