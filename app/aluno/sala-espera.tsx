...
import { handleFirestoreError, safeFirestoreVoidOperation } from '@/utils/firestoreErrorHandler';
...
import { Dimensions, Modal, Pressable, StatusBar, StyleSheet, TextInput, View } from 'react-native';
...
export default function SalaEspera() {
  ...
  async function escolherCorAutomatica() {
    ...
    await safeFirestoreVoidOperation(
      () =>
        setDoc(
          doc(firestore, 'salas', String(params.salaId || ''), 'players', aluno.id),
          novo,
        ),
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      doc(firestore, 'salas', String(params.salaId)),
      (docSnap) => {
        if (docSnap.exists()) {
      ...
      (error) => handleFirestoreError(error, 'ouvir status da sala'),
    );
    return () => unsub();
  }, [params.salaId]);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (error) => handleFirestoreError(error, 'ouvir chat da sala'),
    );
    return () => unsub();
  }, [params.salaId]);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (error) => handleFirestoreError(error, 'ouvir players da sala'),
    );
    return () => unsub();
  }, [params.salaId]);
  ...
  async function enviarFala() {
    ...
    await safeFirestoreVoidOperation(
      () =>
        setDoc(
          doc(firestore, 'salas', String(params.salaId || ''), 'players', player.id),
          { ...player, fala: fala.trim() },
        ),
      'enviar fala',
    );
    setFala('');
  ...
  useEffect(() => {
    return () => {
      if (aluno && params.salaId && typeof params.salaId === 'string') {
        ...
        safeFirestoreVoidOperation(() => deleteDoc(playerRef), 'remover player ao sair');
      }
    };
  }, [aluno, params.salaId]);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (error) => handleFirestoreError(error, 'ouvir instruções da sala'),
    );
    return () => unsub();
  }, [params.salaId]);
