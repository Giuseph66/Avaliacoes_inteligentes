    ...
    try {
      if (salaIdRef.current && alunoRef.current && !liberadoRef.current) {
        ...
        await safeFirestoreVoidOperation(
          () =>
            setDoc(alunoDoc, {
              id: alunoRef.current.id,
              nome: alunoRef.current.nome,
              status: 'fora_sala',
              motivo,
            }),
          'atualizar status fora_sala',
        );
        Alert.alert(
  ...
  useEffect(() => {
    ...
    const buscarProva = async () => {
      try {
        ...
        const provaSnap = await safeFirestoreOperation(() => getDoc(provaRef), 'buscar prova');
        if (!isMounted || !provaSnap) return;
        if (!provaSnap.exists()) {
        ...
        const salaSnap = await safeFirestoreOperation(() => getDoc(salaRef), 'buscar sala');
        if (!salaSnap) return;
        ...
        if (salaSnap.exists() && salaSnap.data().fimProva) {
          ...
          if (provaData.provaCriptografada) {
            try {
              const provaDescriptografada = JSON.parse(base64ToUtf8(provaData.provaCriptografada));
              setProva({
