  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const respSnap = await safeFirestoreOperation(
        () => getDoc(doc(firestore, 'respostas', String(params.respostaId))),
        'buscar resposta',
      );
      ...
      const provaSnap = await safeFirestoreOperation(
        () => getDoc(doc(firestore, 'provas', String(respData.provaId))),
        'buscar prova',
      );
      if (!provaSnap || !provaSnap.exists()) {
        router.back();
        return;
      }
      ...
      if (provaData.provaCriptografada) {
        try {
          const desc = JSON.parse(base64ToUtf8(provaData.provaCriptografada));
