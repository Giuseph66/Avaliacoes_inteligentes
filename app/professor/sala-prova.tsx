...
export default function SalaProva() {
  ...
  useEffect(() => {
    const carregarProfessor = async () => {
      try {
        ...
        Alert.alert('Erro', 'Não foi possível carregar os dados do professor. Verifique sua conexão.');
      }
    };
    carregarProfessor();
  }, []);
      ...
      try {
        ...
        unsubSala = onSnapshot(salaRef, (docSnap) => {
          try {
            ...
            Alert.alert('Erro de Conexão', 'Problema ao carregar dados da sala. Verifique sua internet.');
            setLoading(false);
          }
        }, (error) => {
          console.error('Erro no listener da sala:', error);
          Alert.alert('Erro de Conexão', 'Problema ao conectar com o banco de dados. Verifique sua internet.');
          setLoading(false);
        });
        ...
        unsubAlunos = onSnapshot(collection(firestore,'salas',String(salaDocId),'alunos'), (snap)=>{
          try {
            ...
            Alert.alert('Erro de Conexão', 'Problema ao carregar lista de alunos. Verifique sua internet.');
          }
        }, (error) => {
          console.error('Erro no listener de alunos:', error);
          Alert.alert('Erro de Conexão', 'Problema ao conectar com o banco de dados. Verifique sua internet.');
        ...
        Alert.alert('Erro de Conexão', 'Não foi possível conectar com o banco de dados. Verifique sua internet e tente novamente.');
        setLoading(false);
      }
    })();
    return () => { 
  ...
  async function salvarDataHora() {
    ...
    try {
      ...
      Alert.alert('Sucesso', 'Data e hora salvas!');
    } catch (error) {
      console.error('Erro ao salvar data/hora:', error);
      Alert.alert('Erro', 'Não foi possível salvar a data e hora. Verifique sua conexão.');
    }
  }
  ...
  async function salvarConfiguracoesComVerificacao(date?: Date | null) {
    ...
    try {
      ...
      Alert.alert('Erro', 'Não foi possível salvar as configurações. Verifique sua conexão.');
    } finally {
  ...
  async function liberarSala() {
    if (!salaId) return;
    if (!dataHora) {
      Alert.alert('Atenção', 'Preencha a data e hora de término da prova antes de liberar!');
      return;
    }
    if (!instrucoes || instrucoes.trim().length === 0) {
      Alert.alert('Atenção', 'Preencha as instruções para o aluno antes de liberar!');
      return;
    }
    try {
      await updateDoc(doc(firestore, 'salas', salaId), {
      ...
      Alert.alert('Sucesso', 'Sala liberada para os alunos!');
    } catch (error) {
      console.error('Erro ao liberar sala:', error);
      Alert.alert('Erro', 'Não foi possível liberar a sala. Verifique sua conexão.');
    }
  }
  ...
  async function fechar_abrirSala() {
    ...
    try {
      if (status === 'fechada') {
        ...
        Alert.alert('Sucesso', 'Sala aberta!');
      } else {
        await updateDoc(doc(firestore, 'salas', salaId), { status: 'fechada' });
        Alert.alert('Sucesso', 'Sala fechada!');
      }
    } catch (error) {
      console.error('Erro ao alterar status da sala:', error);
      Alert.alert('Erro', 'Não foi possível alterar o status da sala. Verifique sua conexão.');
    }
  }
  ...
  async function handleShare() {
    ...
    try {
      ...
      Alert.alert('Erro', 'Não foi possível compartilhar o código da sala.');
    }
  }
  ...
  function handleCopy() {
    ...
    try {
      ...
      Alert.alert('Erro', 'Não foi possível copiar o código da sala.');
    }
  }
  ...
  async function removerAluno(id: string, nome: string) {
    ...
    try {
      ...
      Alert.alert('Sucesso', `${nome} foi expulso da sala.`);
    } catch (error) {
      console.error('Erro ao expulsar aluno:', error);
      Alert.alert('Erro', 'Não foi possível expulsar o aluno. Verifique sua conexão.');
    }
  }
  ...
  async function liberarAluno(id: string, nome: string) {
    ...
    try {
      ...
      Alert.alert('Erro', 'Não foi possível liberar o aluno. Verifique sua conexão.');
    }
  }
  ...
  async function liberarAlunoForaSala(id: string, nome: string) {
    ...
    try {
      ...
      console.error('Erro ao liberar aluno fora da sala:', error);
      Alert.alert('Erro', 'Não foi possível liberar o aluno. Verifique sua conexão.');
    }
  }
