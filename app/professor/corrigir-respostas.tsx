...
export default function CorrigirRespostas() {
  ...
  async function openDetalhe(aluno: AlunoFinalizado) {
    setSelected(aluno);
    // Busca resposta com tratamento de erro
    const respSnap = await safeFirestoreOperation(
      () => getDoc(doc(firestore,'respostas',aluno.respostaId)),
    ...
    // Busca prova
    const provaSnap = await safeFirestoreOperation(
      () => getDoc(doc(firestore,'provas',respData.provaId)),
      'buscar prova'
    );
    ...
    if (prova.provaCriptografada){
      try{qs = JSON.parse(base64ToUtf8(prova.provaCriptografada)).questoes||[]}catch{}
    }
    setQuestoes(qs);
    // inicializa notas
  ...
  async function salvarNotas() {
    ...
    try {
      ...
      const okResp = await safeFirestoreVoidOperation(
        () => updateDoc(respostasRef, {
      ...
      if (!okResp) { setSaving(false); return; }
    ...
    try {
      ...
      if (params?.salaId) {
        ...
        await safeFirestoreVoidOperation(
          () => updateDoc(alunoFinalizadoRef, {
            notaGeral: notaGeralFinal,
            corrigidoEm: new Date(),
            corrigidoPorIA: true
          }),
          'salvar nota geral IA'
        );
      }
