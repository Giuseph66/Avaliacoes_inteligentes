  ...
  useEffect(() => {
    async function getProva() {
      if (id) {
        ...
        const provaDoc = await safeFirestoreOperation(() => getDoc(provaRef), 'buscar prova');
        if (provaDoc && provaDoc.exists()) {
          const data = provaDoc.data();
          const provaCriptografada = data.provaCriptografada;
          const provaDescriptografada = JSON.parse(base64ToUtf8(provaCriptografada));
          setProvaData({
