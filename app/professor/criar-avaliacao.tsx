  ...
  useEffect(() => {
    ...
    if (params.id) {
      (async () => {
        ...
        if (snap.exists()) {
          try {
            const decrypted = base64ToUtf8(snap.data().provaCriptografada);
  ...
  async function publicarAvaliacao() {
    ...
    try {
      ...
      if (params.id) {
        ...
        let provaCriptografada = '';
        try {
          console.log('Dados da prova:');
          provaCriptografada = utf8ToBase64(dadosProva);
          console.log('Prova:', prova);
          console.log(provaCriptografada);
        } catch (e) {
          console.log('Erro ao criptografar prova:', e);
        }
